...
app.get("/", async (req, res) => {
    const bots = await Promise.all((await r.table("bots").filter({verified: true}).orderBy(r.desc(doc => doc("servers").doc("likes").mul(50))).limit(13 * 2).run()).map(bot => Util.attachPropBot(bot, req.user)));
