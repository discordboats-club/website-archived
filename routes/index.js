...
app.get('/browse', async (req, res) => {
    ...
    const bots = await Promise.all((await r.table('bots').filter({ verified: true }).orderBy(r.desc('name')).slice(start, start + itemsPerPage)).map(b => Util.attachPropBot(bot, req.user)));
