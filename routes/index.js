...
app.get("/", async (req, res) => {
    const bots = await Promise.all((await r.table("bots").filter({verified: true}).orderBy(r.desc(doc => doc("servers").add(doc("likes")))).limit(13 * 2).run()).map(bot => Util.attachPropBot(bot, req.user)));
