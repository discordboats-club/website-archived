...
app.get('/', async (req, res) => {
    ...
    res.render('dashboard/index', { user: req.user ? await Util.attachPropUser(req.user) : null, myBots: botChunks, rawBots: myBots });
});

app.get('/new', async (req, res) => {
    res.render('dashboard/newBot', { user: req.user ? await Util.attachPropUser(req.user) : null, libs: require('./api').libList });
});
...
app.get('/bot/:id/manage', async (req, res, next) => {
    ...
    if (req.user.id === bot.ownerID || req.user.admin || req.user.mod) {
        ...
        res.render('dashboard/botManage', { bot, user: req.user ? await Util.attachPropUser(req.user) : null });
    } else res.status(403).json({ error: 'you do not own this bot' });
});
...
app.get('/queue', async (req, res) => {
    ...
    res.render('dashboard/queue', { user: req.user ? await Util.attachPropUser(req.user) : null, chunks: botChunks, rawBots: bots, config });
});
